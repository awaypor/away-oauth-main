...
class OAuthLinkController implements RequestHandlerInterface
{
    ...
    private function makeResponse($returnCode = 'done'): HtmlResponse
    {
        if(preg_match('/Android|SymbianOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Windows Phone|Midp/', $_SERVER['HTTP_USER_AGENT'])){
            switch($returnCode){
                case 'already_linked':
                    $message = $this->translator->trans('away-puls-oauth.forum.alerts.already_linked');
                    break;
                case 'already_used':
                    $message = $this->translator->trans('away-puls-oauth.forum.alerts.already_used');
                    break;
                case 'done':
                    $message = $this->translator->trans('away-puls-oauth.forum.alerts.link_success');
                    break;
                case 'error':
                    $message = $this->translator->trans('away-puls-oauth.forum.alerts.error');
                    break;
            }
